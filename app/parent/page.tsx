    ...
    <main className="max-w-lg mx-auto">
      ...
      {pendingDeleteId && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-gray-800 text-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-lg whitespace-nowrap">
          ...
          <button onClick={handleUndoDelete} className="text-amber-400 font-bold text-sm underline">
            {t('undo')}
          </button>
        </div>
      )}
      ...
      <header className="flex items-center justify-between px-4 pt-5 pb-3">
        <div>
          <p className="text-[10px] text-amber-400 font-semibold uppercase tracking-widest">{t('home.family')}</p>
          <h1 className="text-lg font-black text-amber-900 leading-tight">{store.family.name}</h1>
        </div>
        <button onClick={() => router.push('/')} className="text-xs text-amber-400 hover:text-amber-600 transition-colors">
          {t('home.switch')}
        </button>
      </header>
      ...
      {store.kids.length === 0 ? (
        <div className="text-center py-20 px-5">
          ...
          <p className="text-amber-700 font-medium mb-1">{t('home.no-kids')}</p>
          <p className="text-amber-500 text-sm mb-5">{t('home.no-kids-hint')}</p>
          <button
        ...
        <>
          ...
          <div className="flex gap-2 px-4 mb-4">
            ...
            <button
              ...
              className="flex-1 py-3 rounded-2xl bg-emerald-50 text-emerald-600 font-bold text-sm hover:bg-emerald-100 transition-colors shadow-sm border border-emerald-200"
            >
              {t('home.redeem')}
            </button>
          </div>
          ...
          <div className="px-4 pb-6">
            {displayedTxs.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-amber-600 font-medium text-sm">{t('home.no-activity')}</p>
                <p className="text-amber-400 text-xs mt-1">{t('home.no-activity-hint')}</p>
              </div>
            ) : (
              groups.map(group => (
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end" onClick={() => setQuickType(null)}>
          ...
          >
            ...
            <div className={`text-center py-2 rounded-2xl ${
              quickType === 'earn' ? 'bg-amber-50' :
              quickType === 'deduct' ? 'bg-red-50' : 'bg-emerald-50'
            }`}>
              <h2 className={`text-xl font-bold ${
                quickType === 'earn' ? 'text-amber-900' :
            ...
            {store.kids.length > 1 && (
              <div>
                <p className="text-xs font-semibold text-amber-500 uppercase tracking-wide mb-2 text-center">{t('quick.for-kid')}</p>
            ...
            {/* Single kid display */}
            {store.kids.length === 1 && (
              <div className="flex items-center justify-center gap-2 bg-amber-50 rounded-2xl py-2.5">
            ...
            {isEarnOrDeduct && (
              <>
                <p className="text-xs font-semibold text-amber-500 uppercase tracking-wide">{t('quick.select-action')}</p>

                <div className="flex flex-col gap-1.5">
                  {relevantActions.length === 0 && (
                    <p className="text-amber-400 text-sm text-center py-2">{t('quick.no-actions')}</p>
                  )}
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 transition-all text-left ${
                          chosen
                            ? (quickType === 'earn' ? 'border-amber-500 bg-amber-50' : 'border-red-400 bg-red-50')
                            : 'border-amber-100 hover:border-amber-300'
                        }`}
                      >
                        <span className="text-xl flex-shrink-0">{icon}</span>
                        <span className="flex-1 text-sm font-medium text-amber-900">{action.name}</span>
                        <span className={`text-sm font-bold flex-shrink-0 ${quickType === 'earn' ? 'text-amber-500' : 'text-red-500'}`}>
                          {quickType === 'earn' ? '+' : '−'}{action.pointsValue}⭐
                        </span>
                        {chosen && <span className="text-amber-500 font-bold flex-shrink-0">✓</span>}
                      </button>
                    )
                  })}
                  ...
                  >
                    ...
                    <span className="flex-1 text-sm font-medium text-amber-700">{t('quick.custom')}</span>
                    {quickActionId === 'custom' && <span className="text-amber-500 font-bold flex-shrink-0">✓</span>}
                  </button>
                </div>
                ...
                {quickActionId && (
                  <div className="flex flex-col items-center gap-1 pt-1">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className={`text-4xl font-black w-14 text-center ${quickType === 'earn' ? 'text-amber-900' : 'text-red-600'}`}>
                        {quickAmount}
                      </span>
                      <button
                ...
                {!showCreateForm ? (
                  <button
                    ...
                    className="text-center text-amber-400 hover:text-amber-600 text-sm font-medium transition-colors"
                  >
                    {t('quick.create-new')}
                  </button>
                ) : (
                  <div className="bg-amber-50 rounded-2xl p-3 flex flex-col gap-2">
                    <input
                      ...
                      className="rounded-xl border-2 border-amber-200 px-3 py-2 text-amber-900 outline-none focus:border-amber-400 text-sm"
                    />
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-amber-500 font-medium">{t('quick.points')}:</span>
                      <input
                        type="number"
                        min={1}
                        value={createPoints}
                        onChange={e => setCreatePoints(Math.max(1, Number(e.target.value)))}
                        className="w-20 rounded-xl border-2 border-amber-200 px-2 py-1.5 text-amber-900 outline-none focus:border-amber-400 text-center font-bold text-sm"
                      />
                      <button
                        onClick={handleCreateAndSelect}
                        disabled={!createName.trim()}
                        className="flex-1 py-1.5 rounded-xl bg-amber-500 disabled:opacity-40 text-white font-bold text-sm transition-colors"
                      >
                        {t('quick.add-select')}
                      </button>
                      <button
            ...
            {quickType === 'redeem' && (
              <>
                <p className="text-xs font-semibold text-amber-500 uppercase tracking-wide">{t('quick.choose-reward')}</p>

                {activeRewards.length === 0 ? (
                  <p className="text-amber-400 text-sm text-center py-4">{t('quick.no-rewards')}</p>
                ) : (
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center gap-3 px-3 py-3 rounded-xl border-2 ${
                            canAfford ? 'border-emerald-200 bg-emerald-50' : 'border-amber-100 bg-white opacity-70'
                          }`}
                        >
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-amber-900 truncate">{reward.name}</p>
                            <p className="text-xs text-amber-500">{reward.pointsCost}⭐</p>
                          </div>
                          {canAfford ? (
                            <button
            ...
            <button onClick={() => setQuickType(null)} className="text-center text-amber-400 text-sm py-1">
              {t('cancel')}
            </button>
          </div>
        </div>
